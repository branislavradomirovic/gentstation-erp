...
def render(conn):
    ...
    with st.expander("➕ Add New Station", expanded=False):
        ...
        if map_create_data.get("last_clicked"):
            ...
            if (
                st.session_state.get("create_lat") != clicked_lat
                or st.session_state.get("create_lon") != clicked_lon
            ):
        ...
        with st.form("add_station_form"):
            ...
            lat_val = c1.number_input(
                "Latitude", value=st.session_state.get("create_lat", 0.0), format="%.6f"
            )
    ...
    with col_trend_filter:
        trend_date = st.date_input(
            "Select Month (pick any day in month)", value=pd.Timestamp.now()
        )
    ...
    if station_ids:
        ...
        curr = pd.read_sql_query(
            "SELECT * FROM stations WHERE id = %s", conn, params=(sel,)
        ).iloc[0]
        ...
        with st.form(f"edit_station_{sel}"):
            ...
            curr_mgr_name = (
                curr_mgr_name_q["fullname"].iloc[0]
                if not curr_mgr_name_q.empty
                else "-- None --"
            )
            sel_mgr = st.selectbox(
                ...
                mgr_options,
                index=(
                    mgr_options.index(curr_mgr_name)
                    if curr_mgr_name in mgr_options
                    else 0
                ),
            )
            ...
            if st.form_submit_button("Save Station Changes"):
                if not name.strip():
                    ...
                    # LOGGING AND FEEDBACK
                    log_activity(
                        conn, "UPDATE_STATION", f"Updated station ID {sel} ({name})"
                    )
        ...
        if audit_df.empty:
            ...
            for _, report in audit_df.iterrows():
                with st.container(border=True):
                    ...
                    st.markdown(
                        f"**Submitted by:** `{report['employee_name']}` on **{report['timestamp']}**"
                    )
                    ...
                    with score_cols[0]:
                        st.metric(
                            "Safety", f"{report_data.get('safety_score', 'N/A')}/10"
                        )
                    with score_cols[1]:
                        st.metric(
                            "Cleanliness",
                            f"{report_data.get('cleanliness_score', 'N/A')}/10",
                        )
                    with score_cols[2]:
                        st.metric(
                            "Staff", f"{report_data.get('staff_score', 'N/A')}/10"
                        )
                    with score_cols[3]:
                        st.metric(
                            "Merchandising",
                            f"{report_data.get('merchandising_score', 'N/A')}/10",
                        )

                    # Display summary and other details
                    st.markdown(
                        f"**AI Summary:** *{report_data.get('summary', 'No summary available.')}*"
                    )
        ...
        with c_desc:
            ...
            st.caption(
                "Distribute this QR code to employees at this station. Scanning it will open the reporting bot in Telegram."
            )
                ...
                with urllib.request.urlopen(qr_api_url) as response:
                    ...
                    with col_dl:
                        st.download_button(
                            "⬇️ Download QR Code",
                            qr_bytes,
                            file_name=f"station_{sel}_qr.png",
                            mime="image/png",
                            width="stretch",
                        )
            except Exception:
            ...
            with col_email:
                if st.button(
                    "📧 Share via Email",
                    disabled=(not mgr_email),
                    help="Send instructions to Station Manager",
                    width="stretch",
                ):
                    if mgr_email:
                        send_station_qr_email(
                            curr["name"], mgr_email, bot_link, qr_api_url
                        )
                    else:
        ...
        if st.button("🗑️ Delete Station", type="secondary"):
            try:
                ...
                st.error(
                    "Cannot delete station: It contains linked records (e.g., employees, history)."
                )
            except Exception as e:
