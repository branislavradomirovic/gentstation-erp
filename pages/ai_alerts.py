...
def render(conn):
    ...
    if status_filter:
        query += f" AND a.status IN ({','.join(['%s']*len(status_filter))})"
        params.extend(status_filter)
    if severity_filter:
        query += f" AND a.severity IN ({','.join(['%s']*len(severity_filter))})"
        params.extend(severity_filter)
    if station_filter_ids:
        query += f" AND a.station_id IN ({','.join(['%s']*len(station_filter_ids))})"
