...
def render(conn):
    ...
    with get_session() as session:
        # Global Metrics
        total_regions = session.scalar(select(func.count(Region.id)).where(Region.tenant_id == tenant_id))
        total_stations = session.scalar(select(func.count(Station.id)).where(Station.tenant_id == tenant_id))
        total_staff = session.scalar(
            select(func.count(User.id)).where(
                User.tenant_id == tenant_id,
                or_(User.region_id.isnot(None), User.station_id.isnot(None)),
            )
        )
            ...
            with st.form("add_region_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Create Region", use_container_width=True):
                    if not r_name.strip():
                        ...
                        new_reg = Region(tenant_id=tenant_id, name=r_name.strip(), email=r_email.strip() or None)
                        session.add(new_reg)
                        session.commit()
        ...
        ).where(User.tenant_id == tenant_id, User.region_id == Region.id, User.role == 'Region Manager').limit(1).scalar_subquery()

        mgr_id_sub = select(User.id).where(User.tenant_id == tenant_id, User.region_id == Region.id, User.role == 'Region Manager').limit(1).scalar_subquery()
        ...
        stmt = stmt.where(Region.tenant_id == tenant_id)
        ...
        pending_sub = select(func.count(Submission.id)).where(
            Submission.tenant_id == tenant_id, Submission.station_id == Station.id, Submission.processed == 0
        ).scalar_subquery()
    ...
    with tab_edit:
        with st.form(f"edit_region_form_{selected_id}"):
            ...
            if st.form_submit_button("💾 Save Changes", use_container_width=True):
                if not new_name.strip():
                    ...
                    with get_session() as session:
                        session.query(Region).filter(Region.id == selected_id, Region.tenant_id == tenant_id).update({
                            Region.name: new_name.strip(),
                            Region.email: new_email.strip() or None
                        })
                        session.commit()
                    log_activity(conn, "UPDATE_REGION", f"Updated region {selected_id} -> {new_name}") # tenant_id will be picked from current session context
        ...
        if st.button("🗑️ Delete Region", type="secondary", use_container_width=True, key=f"del_region_btn_{selected_id}"):
            with get_session() as session:
                try:
                    session.query(Region).filter(Region.id == selected_id, Region.tenant_id == tenant_id).delete()
                    session.commit()
                    log_activity(conn, "DELETE_REGION", f"Deleted region ID {selected_id}") # tenant_id will be picked from current session context
    ...
    with tab_stations:
        stations_df = pd.read_sql_query(
            "SELECT id as \"ID\", name as \"Station Name\", physical_address as \"Address\" FROM stations WHERE tenant_id = %s AND region_id = %s ORDER BY id",
            conn,
            params=(tenant_id, selected_id),
        )
    ...
    with tab_manager:
        ...
        mgrs = pd.read_sql_query(
            ...
            FROM users
            WHERE tenant_id = %s AND role = 'Region Manager'
            ORDER BY name
            """,
            conn,
            params=(tenant_id,),
        ...
        if st.button("Assign Manager to Region", type="primary", use_container_width=True):
            if selected_mgr == "-- None --":
                ...
                with get_session() as session:
                    session.query(User).filter(User.id == mgr_id, User.tenant_id == tenant_id).update({User.region_id: selected_id})
                    session.commit()
                log_activity(
                    conn,
